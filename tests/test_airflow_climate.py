...
def test_current_temperature_and_humidity(home_assistant: HomeAssistant) -> None:
    ...
    home_assistant.set_state(
        "sensor.airflow_avg_indoor_humidity_5min", "60.0", HUM_ATTRS
    )
...
def test_target_humidity_getter(home_assistant: HomeAssistant) -> None:
    ...
    home_assistant.assert_entity_state(CLIMATE, expected_attributes={"humidity": 62.0})
...
def test_hvac_mode_template(
    home_assistant: HomeAssistant, auto, profile, expected
) -> None:
    if auto:
        _enable_auto(home_assistant)
    else:
...
@pytest.mark.parametrize(
    "profile, free_cool, boost, flush, expected",
    [
...
def test_hvac_action_template(
    home_assistant, profile, free_cool, boost, flush, expected
) -> None:
    _profile(home_assistant, profile)
    home_assistant.set_state(
        "binary_sensor.airflow_free_cooling_available", free_cool, {}
    )
    ...
    home_assistant.assert_entity_state(
        CLIMATE, expected_attributes={"hvac_action": expected}
    )
...
def test_set_temperature_action(home_assistant: HomeAssistant) -> None:
    ...
    home_assistant.call_action(
        "climate", "set_temperature", {"entity_id": CLIMATE, "temperature": 23.0}
    )
    home_assistant.assert_entity_state(
...
def test_set_humidity_action(home_assistant: HomeAssistant) -> None:
    home_assistant.call_action(
        "climate", "set_humidity", {"entity_id": CLIMATE, "humidity": 48}
    )
    home_assistant.assert_entity_state(
...
def test_set_fan_mode_action(home_assistant: HomeAssistant) -> None:
    home_assistant.call_action(
        "climate", "set_fan_mode", {"entity_id": CLIMATE, "fan_mode": "low"}
    )
...
def test_set_swing_mode_action(home_assistant: HomeAssistant) -> None:
    home_assistant.call_action(
        "climate", "set_swing_mode", {"entity_id": CLIMATE, "swing_mode": "both"}
    )
    home_assistant.assert_entity_state("input_select.airflow_swing_mode", "both")


@pytest.mark.parametrize(
    "hvac_mode, expect_auto, expect_profile, expect_state",
    [
        ("auto", "on", None, "auto"),
        ("heat", "off", "warm", "heat"),
        ("cool", "off", "cool", "cool"),
...
def test_set_hvac_mode_action(
    home_assistant, hvac_mode, expect_auto, expect_profile, expect_state
) -> None:
    # Start from a different state so each write is an observable change.
    _enable_auto(home_assistant)
    _profile(home_assistant, "warm")
    home_assistant.call_action(
        "climate", "set_hvac_mode", {"entity_id": CLIMATE, "hvac_mode": hvac_mode}
    )
    home_assistant.assert_entity_state(
